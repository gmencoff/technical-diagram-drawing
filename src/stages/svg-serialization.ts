...
function renderPrimitive(primitive: SvgPrimitive, depth: number): string {
  ...
  switch (primitive.kind) {
    case 'circle':
      return `${indent}<circle cx="${primitive.cx}" cy="${primitive.cy}" r="${primitive.r}"${optAttr('stroke', primitive.stroke)}${optAttr('stroke-dasharray', primitive.strokeDasharray)}${optAttr('fill', primitive.fill)} />`;

    case 'line':
      return `${indent}<line x1="${primitive.x1}" y1="${primitive.y1}" x2="${primitive.x2}" y2="${primitive.y2}"${optAttr('stroke', primitive.stroke)} />`;

    case 'path':
      return `${indent}<path d="${primitive.d}"${optAttr('stroke', primitive.stroke)}${optAttr('fill', primitive.fill)} />`;

    case 'text':
      return `${indent}<text x="${primitive.x}" y="${primitive.y}"${optAttr('font-size', primitive.fontSize?.toString())}${optAttr('text-anchor', primitive.textAnchor)}>${escapeXml(primitive.content)}</text>`;
